...
- [x] **Local base-ingredient database** — **~218** bundled whole foods (fruits/veg/meats incl.
      ground-beef %s/fish & seafood/grains/legumes/dairy incl. plant milks/nuts & seeds/condiments),
      seeded with synthetic `base:<slug>` barcodes, searchable offline (`baseFoodsSeed.ts`,
      `assets/foods/base-ingredients.json`). **Fully enriched** to OFF parity: fiber/sugar/sat-fat/sodium,
      a ~22-nutrient micronutrient breakdown per 100 g, NOVA group, a **computed 2023 Nutri-Score**,
      **allergen badges** (`ALLERGEN_GROUPS`) and **diet labels** (vegan/vegetarian/gluten-free, derived) —
      so base foods open the same rich sheet as scanned products. Macros + micros are pulled from
      **USDA FoodData Central** by `scripts/fetch-base-foods.mjs` (each food stamped with its `fdcId`;
      `scripts/base-foods-fdc.json` lockfile makes the pull verifiable/reproducible, independently
      spot-checked by `scripts/verify-base-foods.py`); NOVA + fruit/veg % stay curated and Nutri-Score
      is computed. Re-seeds in place via `app_meta` `baseFoodsVersion` + `foodRepo.upsertBaseFood`.
- [x] **Bluetooth kitchen scale (live weigh)** — a pluggable `ScaleAdapter` layer (`src/lib/scales/`,
...
- [x] **USDA household portions** — foods carry curated measures (Small/Medium/Large, cup/tbsp/tsp,
      ...
      add-a-food + labeling reference.
- [x] **Quick-add calories** — log a bare calorie (+ optional macros) with no food item, for
      restaurant meals / estimates. Nullable `custom*` columns on `food_logs` (coalesced into every
      totals/range query); a `quick-add` modal reached from the Food FAB + the add-food screen.
- [x] **Copy meal / copy day** — re-log a previous day's items into today from the meal-section kebab
      ("Copy from another day") or the date-header kebab ("Copy entire day"); pick the source day on the
      calendar. `FoodRepo.copyMeal`/`copyDay` re-insert via `addLog` (food/recipe/quick-add all carry over).
- [x] **Saved meals** — save a meal's items as "My usual breakfast" (meal kebab → "Save as meal") and
